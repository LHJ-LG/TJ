...
func TestFFTRoundtrip(t *testing.T) {
	...
	for i := range res {
		if got, expected := &res[i], &data[i]; !kzg_bls.EqualFr(got, expected) {
			t.Errorf("difference: %d: got: %s  expected: %s", i, kzg_bls.FrStr(got), kzg_bls.FrStr(expected))
		}
	}
	t.Log("zero", kzg_bls.FrStr(&kzg_bls.ZERO))
	t.Log("zero", kzg_bls.FrStr(&kzg_bls.ONE))
}
...
func TestInvFFT(t *testing.T) {
	...
	for i := range res {
		if got := &res[i]; !kzg_bls.EqualFr(got, &expected[i]) {
...
func TestEvaluatePolyInEvaluationForm(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		x := kzg_bls.RandomFr()
		var y1 kzg_bls.Fr
		kzg_bls.EvalPolyAt(&y1, data, x)

		var y2 kzg_bls.Fr
		kzg_bls.EvaluatePolyInEvaluationForm(&y2, res, x, fs.ExpandedRootsOfUnity[:fs.MaxWidth], 0)

		if !kzg_bls.EqualFr(&y1, &y2) {
			debugFrs("y", []kzg_bls.Fr{y1, y2})
