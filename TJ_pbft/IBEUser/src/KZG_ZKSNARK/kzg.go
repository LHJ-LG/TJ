...
type KZGSettings struct {
	*FFTSettings
	...
	SecretG1 []kzg_bls.G1Point
	// [b.multiply(b.G2, pow(s, i, MODULUS)) for i in range(WIDTH+1)],
	SecretG2 []kzg_bls.G2Point
}

func NewKZGSettings(fs *FFTSettings, secretG1 []kzg_bls.G1Point, secretG2 []kzg_bls.G2Point) *KZGSettings {
...
func NewFK20SingleSettings(ks *KZGSettings, n2 uint64) *FK20SingleSettings {
	...
	x := make([]kzg_bls.G1Point, n, n)
	for i, j := uint64(0), n-2; i < n-1; i, j = i+1, j-1 {
		kzg_bls.CopyG1(&x[i], &ks.SecretG1[j])
	}
	kzg_bls.CopyG1(&x[n-1], &kzg_bls.ZeroG1)
...
func NewFK20MultiSettings(ks *KZGSettings, n2 uint64, chunkLen uint64) *FK20MultiSettings {
	...
	if !kzg_bls.IsPowerOfTwo(chunkLen) {
	...
	fk := &FK20MultiSettings{
		KZGSettings:  ks,
		chunkLen:     chunkLen,
		xExtFFTFiles: make([][]kzg_bls.G1Point, chunkLen, chunkLen),
	}
	// xext_fft = []
	// for i in range(l):
	...
	n := n2 / 2
	k := n / chunkLen
	xExtFFTPrecompute := func(offset uint64) []kzg_bls.G1Point {
		x := make([]kzg_bls.G1Point, k, k)
		start := n - chunkLen - 1 - offset
		for i, j := uint64(0), start; i+1 < k; i, j = i+1, j-chunkLen {
			kzg_bls.CopyG1(&x[i], &ks.SecretG1[j])
		}
		kzg_bls.CopyG1(&x[k-1], &kzg_bls.ZeroG1)
