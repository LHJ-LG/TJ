...
func integrationTestSetup(scale uint8, seed int64) (data []byte, extended []kzg_bls.Fr, extendedAsPoly []kzg_bls.Fr, commit *kzg_bls.G1Point, ks *KZGSettings) {
	points := 1 << scale
	size := points * 31
	...
	evenPoints := make([]kzg_bls.Fr, points, points)
	...
	for i := 0; i < points; i++ {
		...
		kzg_bls.FrFrom32(&evenPoints[i], tmp)
	}
	reverseBitOrderFr(evenPoints)
	oddPoints := make([]kzg_bls.Fr, points, points)
	for i := 0; i < points; i++ {
		kzg_bls.CopyFr(&oddPoints[i], &evenPoints[i])
	}
	...
	extended = make([]kzg_bls.Fr, points*2, points*2)
	for i := 0; i < len(extended); i += 2 {
		kzg_bls.CopyFr(&extended[i], &evenPoints[i/2])
		kzg_bls.CopyFr(&extended[i+1], &oddPoints[i/2])
	}
...
func TestFullDAS(t *testing.T) {
	...
	for i := uint64(0); i < sampleCount; i++ {
		...
		sample.sub = make([]kzg_bls.Fr, cosetWidth, cosetWidth)
		for j := uint64(0); j < cosetWidth; j++ {
			kzg_bls.CopyFr(&sample.sub[j], &extended[i*cosetWidth+j])
		}
	...
	for i, sample := range samples {
		...
		kzg_bls.CopyFr(&x, &ks.ExpandedRootsOfUnity[domainPos*domainStride])
	...
	partialReconstructed := make([]*kzg_bls.Fr, extSize, extSize)
	...
	for i := 0; i < len(recovered); i++ {
		if !kzg_bls.EqualFr(&extended[i], &recovered[i]) {
			t.Errorf("diff %d: %s <> %s", i, kzg_bls.FrStr(&extended[i]), kzg_bls.FrStr(&recovered[i]))
		}
	}
