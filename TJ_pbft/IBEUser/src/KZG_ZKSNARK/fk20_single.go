...
func (ks *KZGSettings) toeplitzPart1(x []kzg_bls.G1Point) []kzg_bls.G1Point {
	n := uint64(len(x))
	n2 := n * 2
	// Extend x with zeros (neutral element of G1)
	xExt := make([]kzg_bls.G1Point, n2, n2)
	for i := uint64(0); i < n; i++ {
		kzg_bls.CopyG1(&xExt[i], &x[i])
	}
	for i := n; i < n2; i++ {
		kzg_bls.CopyG1(&xExt[i], &kzg_bls.ZeroG1)
	}
...
func (ks *KZGSettings) ToeplitzPart2(toeplitzCoeffs []kzg_bls.Fr, xExtFFT []kzg_bls.G1Point) (hExtFFT []kzg_bls.G1Point) {
	...
	//print("mul n: ", n)
	hExtFFT = make([]kzg_bls.G1Point, n, n)
	for i := uint64(0); i < n; i++ {
		kzg_bls.MulG1(&hExtFFT[i], &xExtFFT[i], &toeplitzCoeffsFFT[i])
	}
...
func (ks *KZGSettings) ToeplitzPart3(hExtFFT []kzg_bls.G1Point) []kzg_bls.G1Point {
...
func (ks *KZGSettings) toeplitzCoeffsStepStrided(polynomial []kzg_bls.Fr, offset uint64, stride uint64) []kzg_bls.Fr {
	n := uint64(len(polynomial))
	k := n / stride
	k2 := k * 2
	// [last poly item] + [0]*(n+1) + [poly items except first and last]
	toeplitzCoeffs := make([]kzg_bls.Fr, k2, k2)
	kzg_bls.CopyFr(&toeplitzCoeffs[0], &polynomial[n-1-offset])
	for i := uint64(1); i <= k+1; i++ {
		kzg_bls.CopyFr(&toeplitzCoeffs[i], &kzg_bls.ZERO)
	}
	for i, j := k+2, 2*stride-offset-1; i < k2; i, j = i+1, j+stride {
		kzg_bls.CopyFr(&toeplitzCoeffs[i], &polynomial[j])
	}
	return toeplitzCoeffs
}

// TODO: call above with offset=0, stride=1
func (ks *KZGSettings) toeplitzCoeffsStep(polynomial []kzg_bls.Fr) []kzg_bls.Fr {
	n := uint64(len(polynomial))
	n2 := n * 2
	// [last poly item] + [0]*(n+1) + [poly items except first and last]
	toeplitzCoeffs := make([]kzg_bls.Fr, n2, n2)
	kzg_bls.CopyFr(&toeplitzCoeffs[0], &polynomial[n-1])
...
func (fk *FK20SingleSettings) FK20Single(polynomial []kzg_bls.Fr) []kzg_bls.G1Point {
...
func (fk *FK20SingleSettings) FK20SingleDAOptimized(polynomial []kzg_bls.Fr) []kzg_bls.G1Point {
...
func (fk *FK20SingleSettings) DAUsingFK20(polynomial []kzg_bls.Fr) []kzg_bls.G1Point {
	n := uint64(len(polynomial))
	if n > fk.MaxWidth/2 {
