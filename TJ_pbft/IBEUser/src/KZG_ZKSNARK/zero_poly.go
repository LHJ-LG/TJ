...
type ZeroPolyFn func(missingIndices []uint64, length uint64) ([]kzg_bls.Fr, []kzg_bls.Fr)

func (fs *FFTSettings) makeZeroPolyMulLeaf(dst []kzg_bls.Fr, indices []uint64, domainStride uint64) {
	...
	for i := len(indices) + 1; i < len(dst); i++ {
		kzg_bls.CopyFr(&dst[i], &kzg_bls.ZERO)
	}
	kzg_bls.CopyFr(&dst[len(indices)], &kzg_bls.ONE)
	var negDi kzg_bls.Fr
	for i, v := range indices {
		kzg_bls.SubModFr(&negDi, &kzg_bls.ZERO, &fs.ExpandedRootsOfUnity[v*domainStride])
		kzg_bls.CopyFr(&dst[i], &negDi)
		if i > 0 {
			kzg_bls.AddModFr(&dst[i], &dst[i], &dst[i-1])
			for j := i - 1; j > 0; j-- {
				kzg_bls.MulModFr(&dst[j], &dst[j], &negDi)
				kzg_bls.AddModFr(&dst[j], &dst[j], &dst[j-1])
			}
			kzg_bls.MulModFr(&dst[0], &dst[0], &negDi)
		}
	}
}

// Copy all of the values of poly into out, and fill the remainder of out with zeroes.
func padPoly(out []kzg_bls.Fr, poly []kzg_bls.Fr) {
	for i := 0; i < len(poly); i++ {
		kzg_bls.CopyFr(&out[i], &poly[i])
	}
	for i := len(poly); i < len(out); i++ {
		kzg_bls.CopyFr(&out[i], &kzg_bls.ZERO)
	}
}
...
func (fs *FFTSettings) reduceLeaves(scratch []kzg_bls.Fr, dst []kzg_bls.Fr, ps [][]kzg_bls.Fr) []kzg_bls.Fr {
	n := uint64(len(dst))
	...
	for i := uint64(0); i < last; i++ {
		p := ps[i]
		for j := 0; j < len(p); j++ {
			kzg_bls.CopyFr(&pPadded[j], &p[j])
		}
		if err := fs.InplaceFFT(pPadded, pEval, false); err != nil {
			panic(err)
		}
		for j := uint64(0); j < n; j++ {
			kzg_bls.MulModFr(&mulEvalPs[j], &mulEvalPs[j], &pEval[j])
		}
	}
...
func (fs *FFTSettings) ZeroPolyViaMultiplication(missingIndices []uint64, length uint64) ([]kzg_bls.Fr, []kzg_bls.Fr) {
	if len(missingIndices) == 0 {
		return make([]kzg_bls.Fr, length, length), make([]kzg_bls.Fr, length, length)
	}
	if length > fs.MaxWidth {
		panic("domain too small for requested length")
	}
	if !kzg_bls.IsPowerOfTwo(length) {
	...
	if uint64(len(missingIndices)) <= perLeaf {
		zeroPoly := make([]kzg_bls.Fr, len(missingIndices)+1, length)
	...
	scratch := make([]kzg_bls.Fr, n*3, n*3)
	...
	if zl := uint64(len(zeroPoly)); zl < length {
		zeroPoly = append(zeroPoly, make([]kzg_bls.Fr, length-zl, length-zl)...)
	} else if zl > length {
